...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # Storage location and number of beans eaten
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            # If you don't hit the wall, you can move on and store the next information
            if not hitsWall:
                nextPos = (nextx, nexty)
...
def cornersHeuristic(state, problem):
    ...
    while not len(unvisited) == 0:
        ...
        for corner in unvisited:
            distances += [(util.manhattanDistance(currentPos, corner), corner)]
...
def foodHeuristic(state, problem):
    ...
    result = 0
    for f in food:
        distance = mazeDistance(f, position, problem.startingGameState)
        if result < distance:
            result = distance
    return result
